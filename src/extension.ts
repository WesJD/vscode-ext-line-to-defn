...
const cssObjectToString = (obj: Record<string, string | number>): string => Object.entries(obj).map(([key, value]) => `${key}:${value};`).join(' ')
...
const centerOfRangePos = (range: vscode.Range): vscode.Position => new vscode.Position(
...
const createRangeForPositions = (pos1: vscode.Position, pos2: vscode.Position): vscode.Range => {
	...
	return new vscode.Range(
		new vscode.Position(minLine, minChar),
		new vscode.Position(maxLine, maxChar),
	)
}
...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(vscode.window.onDidChangeTextEditorSelection(async () => {
		...
		const result: (vscode.Location | vscode.LocationLink)[] = await vscode.commands.executeCommand('vscode.executeDefinitionProvider', document.uri, selectionPos)
		const links: vscode.LocationLink[] = result.filter(elem => "targetUri" in elem && elem.targetUri.scheme === document.uri.scheme && elem.targetUri.path === document.uri.path && elem.targetUri.fsPath === document.uri.fsPath) as vscode.LocationLink[]
		if (links.length === 0) {
